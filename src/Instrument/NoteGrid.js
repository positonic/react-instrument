...
const PianoKey = styled.div`
  height: 20px;
  background-color: ${props => (props.isMiddleC ? 'floralwhite' : 'whitesmoke')};
  width: 100%;
...
class NoteGrid extends React.Component {
  ...
  renderKey(pianoKey, i) {
    if (pianoKey.isSharp) {
      return <PianoKeyHolder key={i}><PianoKeyBlack></PianoKeyBlack></PianoKeyHolder>
    } else {
      if (pianoKey.note.indexOf('C') > -1) {
        return <PianoKeyHolder key={i}><PianoKey
          isMiddleC={(pianoKey.midiNumber === 60)}>{pianoKey.note} </PianoKey></PianoKeyHolder>;
      } else {
        return <PianoKeyHolder key={i}><PianoKey></PianoKey></PianoKeyHolder>;
      }
