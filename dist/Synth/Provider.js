  ...
  (function(_React$Component) {
    ...
    function SynthProvider(props) {
      ...
      _this = _possibleConstructorReturn(
        this,
        _getPrototypeOf(SynthProvider).call(this, props)
      );

      _defineProperty(_assertThisInitialized(_this), "playNoteAtTime", function(
        midiNumber,
        time,
        noteLengthInSeconds
      ) {
        ...
        _this.oscillators = _this.sourceNode.setupOscillators(
      ...
      _defineProperty(_assertThisInitialized(_this), "playNote", function(
        midiNumber
      ) {
        ...
        _this.playingVoices = _this.play(
          midiNumber,
          0,
          null,
          null,
      ...
      _defineProperty(_assertThisInitialized(_this), "stopNote", function(
        midiNumber
      ) {
        _this.playingVoices[midiNumber].forEach(function(voice) {
          if (voice.vca) {
            voice.vca.gain.linearRampToValueAtTime(0, voice.release);
            voice.source.gainNode.gain.linearRampToValueAtTime(
              0,
              voice.release
            );
            setTimeout(function() {
      ...
      _defineProperty(_assertThisInitialized(_this), "stopNoteAtTime", function(
        midiNumber,
        time
      ) {
        _this.playingVoices[midiNumber].forEach(function(voice) {
      ...
      _this.synthOutputChannel = (0, _InstrumentOutput.outputChannel)(
        audioContext,
        filters ? filters : [],
        _this.gainNode
      );
      ...
      _this.playNoteAtTime = _this.playNoteAtTime.bind(
        _assertThisInitialized(_this)
      );
      _this.shouldComponentUpdate = _this.shouldComponentUpdate.bind(
        _assertThisInitialized(_this)
      );
      return _this;
    }
